...
REPORT_URL = 'http://localhost:9993/report_upscale'     # capture_watcher에게 보내는 겁니다
...
async def upscale(path, res):
    ...
    ret = timeline.ApplyGradeFromDRX(
        ...
        "/Users/utylee/Documents/Blackmagic Design/DaVinci Resolve/Blur_48_1.1.2.dpx", 0, clips)
    ...
    while True:
        ...
        time.sleep(2)
        if status['JobStatus'] == '완료' or status['JobStatus'] == 'Complete':
...
async def main():
    log.info(f'this is DavinciResolveUpscale_mac.py')

    if len(sys.argv) > 1:
        log.info(f'args: {sys.argv[1]}, {sys.argv[2]}')
        # mac에서는 굳이 프로세스 기다릴 필요가 없는 것 같습니다 바로 .so dll로 구동하는 듯
        # config.fish: set -gx RESOLVE_SCRIPT_LIB "/Applications/DaVinci Resolve/DaVinci Resolve.app/Contents/Libraries/Fusion/fusionscript.so"
        ...
        while True:
            ...
            for proc in psutil.process_iter():
                ...
                if proc.name().lower() == 'fuscript':
                    # log.info(f'found fuscript.exe!')
                    log.info(f'found fuscript!')
                    bFound = 1
                    break
            if bFound == 1:
                break
...
if __name__ == '__main__':
    ...
    log_path = f'/Users/utylee/davinci.log'
