...
async def proc(a):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(url) as response:
            ...
            async with aiofiles.open('/mnt/d/blizz.html', 'w') as f:
                await f.write(html)

            txt = re.search(
                '.*meta\sname=\"description\"\scontent=\"(.*)\"/><meta\sproperty=\"fb', html)
            # l = re.search('\s(\w+)\s(\w+),\s(\d*)레벨',txt.group(1))
            # l = re.search('-\s(\d+)\s([\w\s]+)\s(\w+)\s(\w+),\s(\d*)레벨', txt.group(1))
            # print(txt.group(1))
            l = re.search('-\s(\d+)\s([\w\s]+),\s(\d*)레벨', txt.group(1))
            lvl = l.group(1) if l else ''
            raceprocls = l.group(2) if l else ''
            ilvl = l.group(3) if l else ''

            # lvl = l.group(1) if l else ''
            # race = l.group(2) if l else ''
            # pro = l.group(3) if l else ''
            # cls = l.group(4) if l else ''
            # ilvl = l.group(5) if l else ''
